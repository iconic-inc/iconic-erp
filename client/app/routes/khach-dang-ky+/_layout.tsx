...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  try {
    if (
      ['/khach-dang-ky/login', '/khach-dang-ky/logout'].includes(url.pathname)
    ) {
      return {};
    }

    if (!auth) {
      return redirect('/khach-dang-ky/login' + `?redirect=${url.pathname}`);
    }
    ...
    // Clear session data
    return redirect(`/khach-dang-ky/login`, {
      headers: {
